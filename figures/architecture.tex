...
% Every label is verified against the code:
%   Profiler        = src/pufguard/analyze_corpus.py
%   Policy Engine   = src/pufguard/run_logic.py
%   Report Generator= src/pufguard/build_report.py
...
\begin{tikzpicture}[
  ...
  procbox/.style = {draw=pufteal, very thick, fill=puflight, rounded corners=2pt,
                    align=center, text width=37mm, minimum height=16mm, inner sep=3pt},
...
\node[cfgbox] (src) {\textbf{Source workbooks}\\ D1--D5 + SHA-256 manifest};
...
\node[procbox, below=10mm of src] (prof) {\textbf{Profiler}\\[2pt] verify hashes, normalize,\\ drop all-empty rows,\\ equivalence classes\\ and risk metrics};
\node[procbox, below=10mm of cfg] (eng) {\textbf{Policy Engine}\\[2pt] ground and solve (Clingo),\\ integrity constraints,\\ exactly one stable model};
\node[procbox, below=10mm of rules] (rep) {\textbf{Report Generator}\\[2pt] merge profiles with\\ decisions, render the\\ Markdown report};

% ---------- generated outputs (gray) ----------
\node[outbox, below=10mm of prof] (o1) {profiles, column inventory,\\ scenarios (CSV);\\ SVG chart; \LaTeX{} macros};
\node[outbox, below=10mm of eng] (o2) {decisions, flags, actions\\ (CSV and JSON trace)};
...
% ---------- flows ----------
% workbooks feed the Profiler; configuration feeds Profiler and Engine;
